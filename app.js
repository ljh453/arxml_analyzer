...
function mergeInteractionData(parsedFiles) {
  ...
  const connectors = dedupeBy(successfulFiles.flatMap((entry) => entry.interaction.connectors), (item) => [item.name, item.providerComponentRef, item.requesterComponentRef, item.providerPortRef, item.requesterPortRef].join("|"));
  ...
  if (!nodes.length) {
    nodes = componentTypes.map((type, index) => ({ id: type.path, path: type.path, name: type.name, instanceName: type.name, kind: type.kind, lane: type.lane, packagePath: type.packagePath, fileName: type.fileName, typePath: type.path, order: Number.isFinite(type.order) ? type.order : index, portCount: type.ports.length }));
  }
  ...
  allConnections.forEach((connection) => {
    ...
    if (sourceNode?.lane === "bswService" && targetNode?.lane === "application") {
      connectedServiceNodeIds.add(sourceNode.id);
    }

    if (targetNode?.lane === "bswService" && sourceNode?.lane === "application") {
      connectedServiceNodeIds.add(targetNode.id);
    }
  });
...
function buildInteractionNode(instance, type, fallbackOrder) {
  ...
  return { id: instance.path || instance.typePath || instance.name, path: instance.path || instance.typePath || instance.name, name: type?.name || shortNameFromRef(instance.typePath) || instance.name, instanceName: instance.name, kind, lane, packagePath: type?.packagePath || instance.packagePath, fileName: type?.fileName || instance.fileName, typePath: instance.typePath, order: Number.isFinite(instance.order) ? instance.order : fallbackOrder, portCount: type?.ports.length || 0 };
}
...
function resolveConnection(connector, nodeMap, portMap, index) {
  ...
  return { id: [connector.name, sourceNode.id, targetNode.id, connector.providerPortRef, connector.requesterPortRef, index].join("|"), name: connector.name, sourceId: sourceNode.id, targetId: targetNode.id, sourceName: sourceNode.name, targetName: targetNode.name, sourceInstance: sourceNode.instanceName, targetInstance: targetNode.instanceName, interfaceName: interfaceName || "-", providerPortName: providerPort?.name || shortNameFromRef(connector.providerPortRef) || "-", requesterPortName: requesterPort?.name || shortNameFromRef(connector.requesterPortRef) || "-", fileName: connector.fileName, compositionPath: connector.compositionPath, order: Number.isFinite(connector.order) ? connector.order : index };
}

function buildInteractionLanes(nodes) {
  return ["application", "bswService", "complex", "other"].map((laneId) => ({ id: laneId, label: INTERACTION_LANE_LABELS[laneId], nodes: nodes.filter((node) => node.lane === laneId) })).filter((lane) => lane.nodes.length > 0);
}
